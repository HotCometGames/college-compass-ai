...
import { TrendingUp, TrendingDown, AlertCircle, CheckCircle2, BookOpen, Trophy, FlaskConical } from "lucide-react";
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <motion.div
          className="glass-card p-6 flex flex-col items-center justify-center gap-3 md:row-span-2"
        ...
        >
          ...
          <p className="text-xs text-muted-foreground text-center max-w-[180px]">
            Based on your profile data, projects, and essays
          </p>
        </motion.div>
      ...
      <motion.div className="glass-card p-6 space-y-5" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="space-y-1.5">
            ...
            <Input
              value={p.name} placeholder="Your name"
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="number" step="0.01" min="0" max="4" value={p.gpaUnweighted || ''}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="number" step="0.01" min="0" max="5" value={p.gpaWeighted || ''}
              placeholder="e.g. 4.5"
          ...
          <div className="space-y-1.5">
            ...
            <Input
              type="number" min="400" max="1600" value={p.satScore || ''}
              placeholder="e.g. 1540"
              onChange={e => updateProfile({ satScore: parseInt(e.target.value) || null })}
              className="h-9 text-sm bg-surface"
            />
          </div>
          <div className="space-y-1.5">
            ...
            <Input
              type="number" min="1" max="36" value={p.actScore || ''}
              placeholder="e.g. 34"
              onChange={e => updateProfile({ actScore: parseInt(e.target.value) || null })}
              className="h-9 text-sm bg-surface"
            />
          </div>
          <div className="space-y-1.5 sm:col-span-2 lg:col-span-3">
            ...
            <Input
              value={p.apClasses.join(', ')}
              placeholder="e.g. AP Calc BC, AP Physics C, AP CS A"
              onChange={e => updateProfile({ apClasses: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
              className="h-9 text-sm bg-surface"
            />
          </div>
        </div>
...
import { FolderKanban, Target, PenTool } from "lucide-react";
